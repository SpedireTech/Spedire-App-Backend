...
//                .exceptionHandling(exceptionHandler -> exceptionHandler.authenticationEntryPoint(customAuthenticationFailureHandler::onAuthenticationFailure))
                ...
                .authorizeHttpRequests(request -> request.requestMatchers("/api/v1/user/verifyPhoneNumber", "/api/v1/user/complete-registration", "/api/v1/sms/verify-otp", "/api/v1/user/forgotPassword", "/api/v1/user/resetPassword", "/api/v1/location/nearbyPlaces").permitAll()
                        .anyRequest()
                        .authenticated()).build();
    }
